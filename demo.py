"""Phase 1 + 2 + 3 demo: run simulations, evaluate them, print a comparison."""
...
def run_and_summarize(name, city, drivers, rides, policy):
    ...
    if rider["pickup_time"]["n"]:
        print(
            ...
            f"p95: {rider['pickup_time']['p95']:.2f}"
        )
    ...
    print(
        ...
        f"opportunity Jain's: {fairness['opportunity_fairness']['jains_index']:.3f}"
    )
    print(
        f"System       - overall utilization: {system['overall_driver_utilization']:.2f}  "
        f"mean alloc time: {system['mean_allocation_compute_time_s'] * 1e6:.1f}us  "
        f"runtime: {system['total_simulation_runtime_s'] * 1000:.1f}ms"
    )
    return metrics
...
def main() -> None:
    ...
    print("\n" + "=" * 70)
    print(
        "Note: single-seed comparison only, for eyeballing. Real "
        "policy comparisons need Phase 7 (multi-seed runner) and "
        "Phase 8 (statistical analysis: CIs, effect sizes)."
    )
